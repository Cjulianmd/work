    ...
    <Container>  
        <ContainerForm>
            <Formulario onSubmit={handleSubmit} >
                <div>Crear nuevo post</div>
                <DivInput>
                    <label htmlFor="url"> Url post</label>
                    <input required  id='url' name='url' type="text" placeholder='Ingresa url de la imagen'
                        value={data.url}
                        onChange={handleChange} />
                </DivInput>
                <DivInput>
                <label htmlFor="description" style={{fontweight:'500'}}> Description post</label>
                    <input style={{paddingTop:'0.5rem'}} id='description' name='description' type="text" placeholder='Descripcion del post'
                        value={data.description}
                        onChange={handleChange} />
                </DivInput>
                <div style={{marginTop: '5rem', width: '100%', padding:'1rem'}}>
                    <button style={{width: '100%'}}>Cargar Post</button>
                </div>
            </Formulario>
        </ContainerForm>
    </Container>
